...
class Util {
	...
	public static function writeCookie($data, $cookietimeout = -1) {
		...
		//$Cookie->domain = "kimlong.com";//Yii::app ()->params->cookie ['domain'];
	...
	public static function getCurrentUrl($no_query = true){
		...
		// Get path to script
		$myUrl .= isset($_SERVER['REDIRECT_URL']) ? $_SERVER['REDIRECT_URL'] : $_SERVER['REQUEST_URI'];
		if (!$no_query){
    ...
    public static function stringToDate($date) {
        $arr = array_reverse(explode('/', $date));
    ...
    public static function stringToDatetime($datetime) {
        $arr = explode(' ', $datetime);
        $date = array_reverse(explode('/', $arr[0]));
        return strtotime(implode('-', $date).' '.$arr[1].':00');
    }

    public static function formatMonth($timestamp) {
        if(!isset($timestamp) || $timestamp == null)
        {
            return date('m/Y');
        }
    ...
    public static function stringToMonth($month) {
        $arr = array_reverse(explode('/', $month));
        return strtotime(implode('-', $arr).'-01');
    }

    public static function stringToMoney($str){
        $arr = explode(' ', $str);
        $number = explode('.', $arr[0]);
    ...
    public static function getHashtag(){
        return parse_url(Util::getCurrentUrl() ,PHP_URL_FRAGMENT);
    }
    ...
    public static function getNameFromNumber($number) {
    ...
    public static function convertDateTime($string){
        list($date, $str) = explode("T", $string);
        list($time, $str1) = explode("+", $str);
